...
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Log.PiBounds` | `Mathlib/Analysis/SpecialFunctions/Log/PiBounds.lean` | project-verified | Lower bounds for `log(4*pi)` and `log(2*pi)` using existing pi/log bounds and a rational logarithm inequality |
...
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Exp.Monomial` | `Mathlib/Analysis/SpecialFunctions/Exp/Monomial.lean` | project-verified | Damped natural monomials decrease from their exact turning point; reuses logarithm-over-power monotonicity, including order zero |
