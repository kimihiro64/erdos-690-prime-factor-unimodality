...
| `PrimeFactorUnimodality.Mathlib.Analysis.Complex.HorizontalIntegral` | `Mathlib/Analysis/Complex/HorizontalIntegral.lean` | project-verified | Horizontal contour transport from Cauchy-Goursat and proved vanishing-edge limits |
...
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Complex.LogStirlingLimit` | `Mathlib/Analysis/SpecialFunctions/Complex/LogStirlingLimit.lean` | project-verified | Exact real factorial normalization and full complex Stirling endpoint limit for every fixed shift |
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Complex.ShiftedReciprocal` | `Mathlib/Analysis/SpecialFunctions/Complex/ShiftedReciprocal.lean` | project-verified | Horizontal reciprocal-power derivative, nonvanishing, continuity, norm majorant and integrability for powers greater than one |
...
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Gamma.BetaNorm` | `Mathlib/Analysis/SpecialFunctions/Gamma/BetaNorm.lean` | project-verified | Beta-integral norm and vertical Gamma-ratio bounds |
...
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Gamma.Measurable` | `Mathlib/Analysis/SpecialFunctions/Gamma/Measurable.lean` | project-verified | Global measurability from the entire reciprocal Gamma function |
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Gamma.NegativeHalfNorm` | `Mathlib/Analysis/SpecialFunctions/Gamma/NegativeHalfNorm.lean` | project-verified | Corrected opposite-line norm bounds, the first-pair edge case and pole-free continuity |
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Gamma.QuarterNorm` | `Mathlib/Analysis/SpecialFunctions/Gamma/QuarterNorm.lean` | project-verified | Compensated quarter-line bound from duplication, with an analytic rational constant |
| `PrimeFactorUnimodality.Mathlib.Analysis.Meromorphic.SimplePoleLimit` | `Mathlib/Analysis/Meromorphic/SimplePoleLimit.lean` | project-verified | A nonzero punctured residue limit determines meromorphic order minus one |
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Gamma.Residue` | `Mathlib/Analysis/SpecialFunctions/Gamma/Residue.lean` | project-verified | All nonpositive-integer Gamma residues, affine transport and exact pole orders |
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Gamma.ShiftNorm` | `Mathlib/Analysis/SpecialFunctions/Gamma/ShiftNorm.lean` | project-verified | Recurrence transports positive-strip norm bounds away from the real axis |
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Gamma.StripBound` | `Mathlib/Analysis/SpecialFunctions/Gamma/StripBound.lean` | project-verified | Euler-integral norm bound and endpoint control on every positive vertical strip |
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Gaussian` | `Mathlib/Analysis/SpecialFunctions/Gaussian.lean` | project-verified | Facade for polynomial Gaussian moments and decay |
...
| `PrimeFactorUnimodality.Mathlib.Analysis.SpecialFunctions.Gaussian.PolynomialDecay` | `Mathlib/Analysis/SpecialFunctions/Gaussian/PolynomialDecay.lean` | project-verified | Vanishing at infinity of every natural power times a decaying Gaussian |
...
| `PrimeFactorUnimodality.Mathlib.Analysis.Fourier.CyclicConvolution` | `Mathlib/Analysis/Fourier/CyclicConvolution.lean` | project-verified | Exact cyclic convolution and positive-shift correlation as shared forward and inverse DFTs, preserving colliding coefficient buckets |
| `PrimeFactorUnimodality.Mathlib.Analysis.Fourier.SincSampling` | `Mathlib/Analysis/Fourier/SincSampling.lean` | project-verified | Sinc integral, modulation, and Poisson sampling-series identity without a band-limit assumption |
| `PrimeFactorUnimodality.Mathlib.Analysis.Fourier.SincError` | `Mathlib/Analysis/Fourier/SincError.lean` | project-verified | Fourier inversion controls sinc interpolation by the actual off-cell tail and integrated alias error |
| `PrimeFactorUnimodality.Mathlib.Analysis.Fourier.ExponentialIntegral` | `Mathlib/Analysis/Fourier/ExponentialIntegral.lean` | project-verified | Exact two-sided exponential integrals and integrability/tail bounds for finite majorants |
| `PrimeFactorUnimodality.Mathlib.Analysis.Fourier.SincExponential` | `Mathlib/Analysis/Fourier/SincExponential.lean` | project-verified | Explicit sinc interpolation error from power-decaying time samples and finite exponential Fourier envelopes |
| `PrimeFactorUnimodality.Mathlib.Analysis.Fourier.SymmetricTruncation` | `Mathlib/Analysis/Fourier/SymmetricTruncation.lean` | project-verified | Symmetric integer-series truncation with a shared tail majorant and finite input-error transport |
| `PrimeFactorUnimodality.Mathlib.Analysis.Calculus.TaylorBound` | `Mathlib/Analysis/Calculus/TaylorBound.lean` | project-verified | Sharp factorial remainder for Banach-valued Taylor approximation, for either orientation of a real segment |
| `PrimeFactorUnimodality.Mathlib.Analysis.PSeries.TailBound` | `Mathlib/Analysis/PSeries/TailBound.lean` | project-verified | Integral-test cutoff bounds and convergence for normed sums dominated by a real power tail |
| `PrimeFactorUnimodality.Mathlib.Analysis` | `Mathlib/Analysis.lean` | project-verified | Bounded analytic facade preserving the previous complex, Fourier and special-function exports while adding calculus bounds |
