...
/-! # The bootstrap with no moment or mass-sum inputs

Exercise the actual sixteen-band theorem using only its elementary budget,
static conditions, and zero-location hypotheses. The scalar inequalities
are assumptions of this interface regression, not numerical certificates.
-/
...
example
  ...
  exact xi_zero_gap_of_elementary_grid_budget hR hr hT hH hn hθ hηdef hηhalf
    hσdef hσ₀ hδ hκ hκ₁ hκ₂ hκ₃ hc hgap hcut hy hdy hend
    a ha ha₀ ha₁ hpoly hlow hhigh hbudget 16 (by norm_num)
    height hheight hthreshold hmargin p hp
