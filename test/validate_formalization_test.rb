...
class ValidateFormalizationTest < Minitest::Test
  ...
  CliStatus = Struct.new(:exitstatus) do
    def success?
      exitstatus.zero?
    end
  end
  ...
  def cli(*arguments)
    output = StringIO.new
    errors = StringIO.new
    exitstatus = FormalizationTemplate.run_cli(arguments.dup, output: output, errors: errors)
    [output.string, errors.string, CliStatus.new(exitstatus)]
  end
