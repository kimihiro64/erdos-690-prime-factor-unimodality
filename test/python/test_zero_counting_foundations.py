...
def test_rosser_frozen_ci_build_order() -> None:
    root = Path(__file__).resolve().parents[2]
    workflow = (root / ".github/workflows/ci.yml").read_text()
    foundations = workflow.split("  lean-foundations:", 1)[1].split("  certificate-prebuild:", 1)[0]
    prefix = "PrimeFactorUnimodality.Helpers.Analytic"
    previous = foundations.index("lake env lean test/lean/DusartBoxRosserTheta.lean")
    for module in (
        "DusartBoxRosserFrozenTail",
        "DusartBoxRosserFrozenAverage",
        "DusartBoxRosserFrozenPointwise",
    ):
        build = foundations.index(f"lake build \\\n            +{prefix}.{module}\n")
        regression = foundations.index(f"lake env lean test/lean/{module}.lean")
        assert previous < build < regression
...
def test_split_low_ci_build_order() -> None:
    root = Path(__file__).resolve().parents[2]
    workflow = (root / ".github/workflows/ci.yml").read_text()
    foundations = workflow.split("  lean-foundations:", 1)[1].split("  certificate-prebuild:", 1)[0]
    prefix = "PrimeFactorUnimodality.Helpers.Analytic"
    previous = foundations.index("lake env lean test/lean/DusartBoxRosserFrozenPointwise.lean")
    for module in ("DusartBoxLowSplit", "DusartBoxSplitAverage", "DusartBoxSplitPointwise"):
        build = foundations.index(f"lake build \\\n            +{prefix}.{module}\n")
        regression = foundations.index(f"lake env lean test/lean/{module}.lean")
        assert previous < build < regression
        previous = regression
    assert previous < foundations.index(f"+{prefix}.ZetaExplicitBounds\n")
    assert foundations.index(f"+{prefix}.XiRosserClosedBounds\n") < foundations.index(
        "lake env lean test/lean/XiRosserPowerTail.lean"
    )
