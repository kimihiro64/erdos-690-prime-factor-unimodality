...
import PrimeFactorUnimodality.Proof.FiniteRange.Generated.K17

set_option autoImplicit false
/-! Generated exact certificate for non-unimodality at `k = 18`. -/
...
/-- Exact strict-valley data proving non-unimodality for `k = 18`. -/
def k18ExplicitRatioValley : ExplicitRatioValleyCertificate 16 where
  descentPrime := 887
  descentNext := 907
  ascentPrime := 907
  ascentNext := 911
  ...
  ratioDefinedAtDescent := by
    apply le_trans (show 17 ≤ Nat.primeCounting' 60 by decide)
  ...
  descentRatio := by
    rw [densityRatio_perm primesBelow887_perm,
      densityRatio_eq_selectionNumerator_ratio primePrefix887
        primePrefix887_gt_one 17,
      ← selectionRowBounded_getD 18 primePrefix887 (17 - 1) (by omega),
      ← selectionRowBounded_getD 18 primePrefix887 17 (by omega),
      numericalRow887]
    norm_num
  ascentRatio := by
    rw [densityRatio_perm primesBelow907_perm,
      densityRatio_eq_selectionNumerator_ratio primePrefix907
        primePrefix907_gt_one 17,
      ← selectionRowBounded_getD 18 primePrefix907 (17 - 1) (by omega),
      ← selectionRowBounded_getD 18 primePrefix907 17 (by omega),
      numericalRow907]
    norm_num

theorem k18PrimeFactorDensity_not_isUnimodal :
    ¬ IsUnimodal (primeFactorDensity 18) := by
  simpa using k18ExplicitRatioValley.not_isUnimodal
