...
import PrimeFactorUnimodality.Proof.FiniteRange.Generated.K16

set_option autoImplicit false
/-! Generated exact certificate for non-unimodality at `k = 17`. -/
...
/-- Exact strict-valley data proving non-unimodality for `k = 17`. -/
def k17ExplicitRatioValley : ExplicitRatioValleyCertificate 15 where
  descentPrime := 887
  descentNext := 907
  ascentPrime := 907
  ascentNext := 911
  ...
  ratioDefinedAtDescent := by
    apply le_trans (show 16 ≤ Nat.primeCounting' 54 by decide)
  ...
  descentRatio := by
    rw [densityRatio_perm primesBelow887_perm,
      densityRatio_eq_selectionNumerator_ratio primePrefix887
        primePrefix887_gt_one 16,
      ← selectionRowBounded_getD 18 primePrefix887 (16 - 1) (by omega),
      ← selectionRowBounded_getD 18 primePrefix887 16 (by omega),
      numericalRow887]
    norm_num
  ascentRatio := by
    rw [densityRatio_perm primesBelow907_perm,
      densityRatio_eq_selectionNumerator_ratio primePrefix907
        primePrefix907_gt_one 16,
      ← selectionRowBounded_getD 18 primePrefix907 (16 - 1) (by omega),
      ← selectionRowBounded_getD 18 primePrefix907 16 (by omega),
      numericalRow907]
    norm_num

theorem k17PrimeFactorDensity_not_isUnimodal :
    ¬ IsUnimodal (primeFactorDensity 17) := by
  simpa using k17ExplicitRatioValley.not_isUnimodal
