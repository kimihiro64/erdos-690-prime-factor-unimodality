...
import PrimeFactorUnimodality.Proof.FiniteRange.Generated.K15

set_option autoImplicit false
/-! Generated exact certificate for non-unimodality at `k = 16`. -/
...
/-- Exact strict-valley data proving non-unimodality for `k = 16`. -/
def k16ExplicitRatioValley : ExplicitRatioValleyCertificate 14 where
  descentPrime := 523
  descentNext := 541
  ascentPrime := 541
  ascentNext := 547
  ...
  ratioDefinedAtDescent := by
    apply le_trans (show 15 ≤ Nat.primeCounting' 48 by decide)
  ...
  descentRatio := by
    rw [densityRatio_perm primesBelow523_perm,
      densityRatio_eq_selectionNumerator_ratio primePrefix523
        primePrefix523_gt_one 15,
      ← selectionRowBounded_getD 15 primePrefix523 (15 - 1) (by omega),
      ← selectionRowBounded_getD 15 primePrefix523 15 (by omega),
      numericalRow523]
    norm_num
  ascentRatio := by
    rw [densityRatio_perm primesBelow541_perm,
      densityRatio_eq_selectionNumerator_ratio primePrefix541
        primePrefix541_gt_one 15,
      ← selectionRowBounded_getD 15 primePrefix541 (15 - 1) (by omega),
      ← selectionRowBounded_getD 15 primePrefix541 15 (by omega),
      numericalRow541]
    norm_num

theorem k16PrimeFactorDensity_not_isUnimodal :
    ¬ IsUnimodal (primeFactorDensity 16) := by
  simpa using k16ExplicitRatioValley.not_isUnimodal
