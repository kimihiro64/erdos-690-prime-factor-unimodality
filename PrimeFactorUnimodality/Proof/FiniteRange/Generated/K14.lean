...
import PrimeFactorUnimodality.Proof.FiniteRange.Generated.K13

set_option autoImplicit false
/-! Generated exact certificate for non-unimodality at `k = 14`. -/
...
/-- Exact strict-valley data proving non-unimodality for `k = 14`. -/
def k14ExplicitRatioValley : ExplicitRatioValleyCertificate 12 where
  descentPrime := 523
  descentNext := 541
  ascentPrime := 541
  ascentNext := 547
  ...
  ratioDefinedAtDescent := by
    apply le_trans (show 13 ≤ Nat.primeCounting' 42 by decide)
  ...
  descentRatio := by
    rw [densityRatio_perm primesBelow523_perm,
      densityRatio_eq_selectionNumerator_ratio primePrefix523
        primePrefix523_gt_one 13,
      ← selectionRowBounded_getD 15 primePrefix523 (13 - 1) (by omega),
      ← selectionRowBounded_getD 15 primePrefix523 13 (by omega),
      numericalRow523]
    norm_num
  ascentRatio := by
    rw [densityRatio_perm primesBelow541_perm,
      densityRatio_eq_selectionNumerator_ratio primePrefix541
        primePrefix541_gt_one 13,
      ← selectionRowBounded_getD 15 primePrefix541 (13 - 1) (by omega),
      ← selectionRowBounded_getD 15 primePrefix541 13 (by omega),
      numericalRow541]
    norm_num

theorem k14PrimeFactorDensity_not_isUnimodal :
    ¬ IsUnimodal (primeFactorDensity 14) := by
  simpa using k14ExplicitRatioValley.not_isUnimodal
