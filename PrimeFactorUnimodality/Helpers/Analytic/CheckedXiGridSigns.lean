...
/-- Opposite checked signs on an exact rational grid give the original sign-row predicate. -/
theorem XiSignRow.valid_of_checked_grid {c : SharedXiGridCheck} (hc : c.Valid)
  ...
  have hpos := SharedXiGridCheck.sign_of_check hc p hp
  have hneg := SharedXiGridCheck.sign_of_check hc q hq
