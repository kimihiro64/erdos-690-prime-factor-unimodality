...
/-- The exact theta coefficient follows from the actual psi estimate, without a scalar input. -/
theorem abs_theta_sub_le_logSquared_far {x : ℝ} (hx : 1 < x) (hb : 5000 ≤ Real.log x)
  ...
  have hp := abs_psi_sub_div_le_far hx hb hgap hreg
...
/-- The far-ray provider is ready for the exact prime-counting and short-interval adapters. -/
theorem hasThetaLogSquaredError_far
  ...
  rw [log_exp] at hb
  exact abs_theta_sub_le_logSquared_far hx1 hb hgap hreg
