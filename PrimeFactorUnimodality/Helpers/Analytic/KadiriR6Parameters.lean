...
/-- Common choices for three-step chains to six, valid at every height at least 25000000. -/
def kadiriSixRow : KadiriEndpointRow :=
  ⟨185573 / 100000, 62 / 100, 437 / 1000, 100000, 109 / 100⟩

/-- The reduced-height initial region follows from the proved bounded-strip gap. -/
...
/-- Every larger cutoff gives at least the reduced endpoint logarithm. -/
theorem kadiriSix_log_height_ge {T : ℝ} (hT : 25000000 ≤ T) :
    (17034 / 1000 : ℝ) ≤ log T :=
  dusart_log_outer_reduced_bounds.1.trans (log_le_log (by norm_num) hT)

/-- The complete harmonic cutoff has a uniform logarithmic lower bound. -/
theorem kadiriSix_log_harmonic_ge {T : ℝ} (hT : 25000000 ≤ T) :
    (99 / 5 : ℝ) ≤ log (16 * T + 100000) := by
  have he : log (16 * 25000000 : ℝ) = 4 * log 2 + log 25000000 := by
    rw [show (16 : ℝ) = 2 ^ 4 by norm_num, log_mul (by norm_num) (by norm_num), log_pow]
    norm_num
  have hlo := log_le_log (by norm_num : (0 : ℝ) < 16 * 25000000)
    (by linarith : (16 * 25000000 : ℝ) ≤ 16 * T + 100000)
  linarith [dusart_log_outer_reduced_bounds.1, log_two_gt_d9]

/-- One scale bound applies at the reduced cutoff and every higher cutoff. -/
theorem kadiriSix_scale_bounds {T r : ℝ} (hT : 25000000 ≤ T) (hr : 6 ≤ r) :
    0 < KadiriEndpointRow.scale r T ∧
      KadiriEndpointRow.scale r T ≤ (98 / 10000 : ℝ) := by
  have hlog := kadiriSix_log_height_ge hT
  have hden : 0 < r * log T := mul_pos (by linarith) (by linarith)
  constructor
...
/-- A single real-coordinate range covers all improving rows above the reduced cutoff. -/
theorem kadiriSix_sigma_bounds {T R : ℝ} (hT : 25000000 ≤ T) (hR : 13 / 2 ≤ R) :
    (9922 / 10000 : ℝ) ≤ kadiriSixRow.sigma R T 16 ∧
      kadiriSixRow.sigma R T 16 ≤ 1 := by
  have hlog := kadiriSix_log_harmonic_ge hT
  have hden : 0 < R * log (16 * T + 100000) := mul_pos (by linarith) (by linarith)
  have hm := mul_le_mul hR hlog (by norm_num) (by linarith)
  have hfrac : 1 / (R * log (16 * T + 100000)) ≤ (78 / 10000 : ℝ) := by
    apply (div_le_iff₀ hden).mpr
    nlinarith only [hm]
  change (9922 / 10000 : ℝ) ≤ 1 - 1 / (R * log (16 * T + 100000)) ∧
    1 - 1 / (R * log (16 * T + 100000)) ≤ 1
  constructor
  · linarith
  · linarith [one_div_pos.mpr hden]

/-- Coarse enclosures suffice for both exact kappa constraints, uniformly in the endpoint scale. -/
theorem kadiriSix_kappa_bounds {η σ : ℝ} (hη : 0 ≤ η) (hη' : η ≤ 98 / 10000)
    (hσ : 9922 / 10000 ≤ σ)
  ...
  let b := 2 * σ - 1
  have hb : (4922 / 5000 : ℝ) ≤ b := by dsimp [b]; linarith
  have hbpos : 0 < b := by linarith
  have hgpos : 0 < g := by dsimp [g]; linarith
  have hmpos : 0 ≤ m := neg_kadiriKernel₂_zero_nonneg mossinghoffTrudgian_theta_mem
  have hηm : η ^ 2 * m ≤ (98 / 10000 : ℝ) ^ 2 * 1163 := by
    apply mul_le_mul _ hm hmpos (by positivity)
    nlinarith only [hη, hη']
  have hnum : (1364 / 10 : ℝ) ≤ g * b - η ^ 2 * m / b := by
    have hmain := mul_le_mul hg hb (by norm_num) hgpos.le
    have herr : η ^ 2 * m / b ≤ ((98 / 10000 : ℝ) ^ 2 * 1163) / (4922 / 5000) := by
      calc
        _ ≤ ((98 / 10000 : ℝ) ^ 2 * 1163) / b := div_le_div_of_nonneg_right hηm hbpos.le
        _ ≤ _ := div_le_div_of_nonneg_left (by norm_num) (by norm_num) hb
    dsimp [g] at hmain ⊢
    norm_num at herr
    linarith
  have hi₁ : 1 / ((62 / 100 : ℝ) + b) ≤ 1 / ((62 / 100 : ℝ) + 4922 / 5000) := by
    apply one_div_le_one_div_of_le (by norm_num)
    linarith
  have hi₂ : 1 / ((62 / 100 : ℝ) + b) ^ 2 ≤
      1 / ((62 / 100 : ℝ) + 4922 / 5000) ^ 2 := by
    apply one_div_le_one_div_of_le (by norm_num)
    gcongr
  have hi₃ : 1 / ((62 / 100 : ℝ) + b) ^ 3 ≤
      1 / ((62 / 100 : ℝ) + 4922 / 5000) ^ 3 := by
    apply one_div_le_one_div_of_le (by norm_num)
    gcongr
  have hden₂ : (1 + 2 * (62 / 100 : ℝ)) * g +
      (1 / (62 / 100 : ℝ) + 1 / ((62 / 100 : ℝ) + b)) * (η ^ 2 * m) ≤ 31175 / 100 := by
    ...
    have hsecond := mul_le_mul (add_le_add (le_refl (1 / (62 / 100 : ℝ))) hi₁) hηm
      (by positivity) (by norm_num : (0 : ℝ) ≤ 1 / (62 / 100) + 1 / (62 / 100 + 4922 / 5000))
    dsimp [g] at *
  ...
  have hden₃ : (1 / (62 / 100 : ℝ) + (1 + (62 / 100 : ℝ)) / ((62 / 100 : ℝ) + b) ^ 2) * g +
    ...
    have hpart : (1 + (62 / 100 : ℝ)) / ((62 / 100 : ℝ) + b) ^ 2 ≤
        (1 + (62 / 100 : ℝ)) / ((62 / 100 : ℝ) + 4922 / 5000) ^ 2 := by
    ...
    have hfirst := mul_le_mul (add_le_add (le_refl (1 / (62 / 100 : ℝ))) hpart) hg' hgpos.le
      (by norm_num : (0 : ℝ) ≤ 1 / (62 / 100) + (1 + (62 / 100)) / (62 / 100 + 4922 / 5000) ^ 2)
    have hsecond := mul_le_mul (add_le_add (le_refl (1 / (62 / 100 : ℝ) ^ 3)) hi₃) hηm
      (by positivity)
      (by norm_num : (0 : ℝ) ≤ 1 / (62 / 100) ^ 3 + 1 / (62 / 100 + 4922 / 5000) ^ 3)
    norm_num at hfirst hsecond ⊢
    linarith
  change (437 / 1000 : ℝ) ≤ (g * b - η ^ 2 * m / b) /
    ((1 + 2 * (62 / 100)) * g + (1 / (62 / 100) + 1 / (62 / 100 + b)) * (η ^ 2 * m)) ∧
    (437 / 1000 : ℝ) ≤ (g * b - η ^ 2 * m / b) /
...
theorem kadiriSix_valid {T R r : ℝ} (hT : 25000000 ≤ T) (hR : 13 / 2 ≤ R) (hr : 6 ≤ r)
    (hg : (1387 / 10 : ℝ) ≤ kadiriKernel (185573 / 100000) 0 ∧
  ...
  obtain ⟨hσ, hσ'⟩ := kadiriSix_sigma_bounds hT hR
  obtain ⟨hk₂, hk₃⟩ := kadiriSix_kappa_bounds hη.le hη' hσ hg.1 hg.2 hm
  refine ⟨by linarith, by linarith, by linarith, by norm_num [kadiriSixRow], by norm_num,
  ...
  · change 0 < kadiriSixRow.sigma R T 16 -
      KadiriEndpointRow.scale r T + 62 / 100
    linarith
  · change 0 ≤ 2 * kadiriSixRow.sigma R T 16 - 1 - (437 / 1000) * (1 + 2 * (62 / 100))
    linarith
  · change (437 / 1000) * (1 + 2 * (62 / 100)) + 1 - kadiriSixRow.sigma R T 16 ≤
      (2 * kadiriSixRow.sigma R T 16 - 1 - (437 / 1000) * (1 + 2 * (62 / 100))) *
        100000 ^ 2
    linarith
