...
/-! # The complete analytic theta ray from ten billion

The two early intervals join the existing infinite ray without strengthening
its zero-data assumptions. Consequently the published cutoff needs finite
theta data only up to ten billion; the coefficient and conclusion are unchanged.
-/
...
/-- Analytic coverage now starts at ten billion and extends through every larger real point. -/
theorem hasThetaLogSquaredError_above_earlier_cutoff
  ...
  · exact abs_theta_sub_le_logSquared_early_bridge hx h (dusartEarlier_low_zero_restriction hgap)
  · exact hasThetaLogSquaredError_above_endpoint hgap hreg x (le_of_not_ge h)

/-- The exact published theorem needs a 144-times shorter finite input. -/
