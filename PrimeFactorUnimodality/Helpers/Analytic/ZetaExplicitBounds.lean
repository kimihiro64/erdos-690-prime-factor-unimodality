...
theorem riemannZeta_norm_real_le_two_div_sub_one
    {σ : Real} (hσ₁ : 1 < σ) (hσ₂ : σ ≤ 2) :
    ‖riemannZeta (σ : Complex)‖ ≤ 2 / (σ - 1) := by
  apply (norm_riemannZeta_real_le_pole_add_one hσ₁).trans
  apply (le_div_iff₀ (sub_pos.mpr hσ₁)).mpr
  have heq : (1 / (σ - 1) + 1) * (σ - 1) = σ := by
...
theorem riemannZeta_norm_real_le_three_div_sub_one
  ...
  exact (riemannZeta_norm_real_le_two_div_sub_one hσ₁ hσ₂).trans
    (div_le_div_of_nonneg_right (by norm_num) (sub_pos.mpr hσ₁).le)
...
theorem zetaNear1BndExact_explicit :
  ...
  intro σ hσ
  exact riemannZeta_norm_real_le_two_div_sub_one hσ.1 hσ.2
