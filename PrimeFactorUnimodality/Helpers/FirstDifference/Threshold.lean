...
/-- Strict descent is equivalently controlled by the reverse strict ratio
comparison. -/
theorem densityStep_lt_iff_symmetricRatio_lt_gap
    (smallerPrimes : List Nat) (r p gap : Nat)
    (hp : 0 < p) (hDensity : 0 < finiteDensity smallerPrimes (r + 1)) :
    prescribedFactorDensity (p :: smallerPrimes) (r + 2) (p + gap) <
          prescribedFactorDensity smallerPrimes (r + 2) p ↔
  ...
  have hNextRat : (0 : Rat) < p + gap := by positivity
  push_cast
  rw [div_lt_div_iff₀ hNextRat hpRat, div_lt_iff₀ hDensity,
    finiteDensity_cons_succ]
  have hpOne : 1 ≤ p := hp
  rw [Nat.cast_sub hpOne]
  field_simp [ne_of_gt hpRat]
...
/-- Weak descent, including a flat step, is controlled by the corresponding
weak ratio comparison. -/
theorem densityStep_le_iff_symmetricRatio_le_gap
    (smallerPrimes : List Nat) (r p gap : Nat)
    (hp : 0 < p) (hDensity : 0 < finiteDensity smallerPrimes (r + 1)) :
    prescribedFactorDensity (p :: smallerPrimes) (r + 2) (p + gap) ≤
          prescribedFactorDensity smallerPrimes (r + 2) p ↔
      densityRatio smallerPrimes (r + 1) ≤ (gap + 1 : Nat) := by
  rw [← not_lt, ← not_lt]
  exact not_congr (densityStep_iff_symmetricRatio_gap smallerPrimes r p gap hp hDensity)
